...
{- Adds the specified Tetromino to a Board -}     
...
{- Checks to see if the Tetromino is within the bounds of a board -}     
inBounds : Tetromino -> Bool
inBounds { shape } =
  let checkLocation (r, c) = r >= 0 && c >= 0 && c < cols
  in List.all checkLocation shape

{- Checks to see if the specified Tetromino intersects with any of
...
{- Checks to see if the Tetromino and Board are valid in a game -}     
